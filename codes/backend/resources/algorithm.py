...
def checker(image_path,A=-1,B=-1,X=-1,Y=-1, acc_thresh=0.60):
    ...
    if(type(img_i) != np.ndarray):
        return {"response" : "Invalid Image passed" }
    ...
    if(asp_ratio != 1.4):
        print('Poor Image Aspect Ratio : Results Will be affected',file=sys.stderr)
    ...
    if (asp_h > 4676):#Oversized image
        ...
        print('Image less than 300 dpi might have reduced accuracy',file=sys.stderr)
    else:
        img = img_i
    ...
    if(len(workspaces) != 3):
        print('Invalid worksheet image passed, please scan properly',file=sys.stderr)
        return { "response" : "Invalid worksheet image passed, please scan properly" }
